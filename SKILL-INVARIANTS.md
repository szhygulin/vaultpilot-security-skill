...
2. **Integrity pin.** `SKILL.md` carries a sentinel near the top (current:
   `VAULTPILOT_PREFLIGHT_INTEGRITY_v17_accad44c20cf5141`) and
...
| **#6 — Cross-chain on-chain destination IDs** | Rogue MCP swaps `destinationChainId` in a LiFi swap (e.g., user says "bridge to Polygon", MCP encodes Optimism-chain attacker contract). | Destination selected only from user intent; TRON accepts `1885080386571452`, never its API ID `728126428`. Bridge labels cannot waive a mismatch. Does not prove recipient or opaque facet routing — see #6b. |
| **#6b — Tier-1 facet decoder** | Bridge facet recipient swap (smoke-test 136, 137). Outer LiFi `BridgeData` passes #6 cleanly (correct destination chain ID) but attacker recipient lives one decode-layer deeper inside opaque facet data; the Ledger ETH app blind-signs LiFi calldata. | Tier-1 only in v0.6.0 (Wormhole / Mayan / NEAR Intents / Across V3). Tier-2 (deBridge / Stargate `composeMsg` / Hop / Symbiosis) falls back to a recommended (optional) second-LLM check (#12.5) until shipped — tracked at [#451](https://github.com/szhygulin/vaultpilot-mcp/issues/451). |
...
| **#12.5 — High-blast-radius ops list (second-LLM recommended)** | High-blast-radius ops where agent-only review is insufficient: 7702 `setCode`, Permit2 batch with multi-month expirations, LiFi Tier-2 bridges, multi-row revoke, `prepare_safe_tx_*` with `enableModule` / `delegateCall: true`, **`prepare_custom_call` (opaque calldata against non-protocol target)**, **any `prepare_*` whose VERIFY block carries `verification.humanDecode.source === "none"`**, signing flows targeting addresses outside #1.a + saved contacts. The agent names the op class and recommends the second-LLM check before 'send'; the check is always optional and the user may decline and proceed. A user who runs it and gets a decode that disagrees with the agent's narrative is told not to sign. The skill's trigger for `prepare_custom_call` is the **tool name** the agent observed (outside the MCP's reach), so the recommendation fires under rogue-MCP regardless of what the MCP reports. | Advisory, not a gate: a user who declines gets no second-LLM coverage. A rule that blocked 'send' on the check would block the user, not an attacker — a rogue agent self-attests the verdict either way. When the check does run, fabricating its verdict requires a coordinated lie across two independent provider sessions — significantly harder than a single-agent compromise. The `humanDecode.source === "none"` trigger is defense-in-depth (cooperating-agent + honest-MCP); the tool-name trigger covers the rogue-MCP case. |
...
## Cooperating-agent guidance (v0.7.0 + v0.8.0 + v0.9.0 + v0.10.0 + v0.11.0 + v0.12.0 + v0.13.0 + v0.14.0)
...
- **v0.14.0 — Second-LLM check is always optional.** Retires
  v0.13.0's refusal gate: Inv #12.5 names the high-blast-radius op
  classes and recommends the check, and no op class makes it a
  condition of `preview_send` / `send_transaction`. The disagreement
  halt stays — a user who runs the check and gets a contradicting
  decode is still told not to sign. The `secondLlmRequired`
  references are gone; the MCP removed that field in
...
README roadmap; Tier-2 routes fall back to a recommended (optional)
second-LLM check (#12.5) until shipped.
